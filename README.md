...
Sitio estático de Piñeyro y Moscatelli, Servicio Oficial Toyota.
...
Las fichas dedicadas se generan desde una única plantilla en `templates/model.html` y el catálogo de datos en `data/models.json`. El generador `scripts/generate-model-pages.mjs` valida los campos obligatorios, resuelve todos los placeholders y escribe una página HTML por cada modelo; el `build` lo ejecuta automáticamente antes de crear `dist/`. El CSS común de las fichas vive en `toyota-sharp-assets/model-ficha.css`, por lo que los cambios visuales no deben copiarse entre páginas.

Para regenerar las fichas sin construir todo el sitio:

```bash
node scripts/generate-model-pages.mjs
```

Al agregar una ficha, incorporá un objeto completo en `data/models.json` con `slug` (URL) y `slugJs` (identificador JavaScript seguro), además de sus metadatos, imágenes y especificaciones. Las secciones específicas de carrocería siguen siendo contenido de la plantilla hasta que se incorporen variantes opcionales de sección.
...
Solo se publica `dist/`, que genera `scripts/build.sh`: `index.html`, las fichas HTML generadas, `404.html`, `toyota-sharp-assets/` y `.nojekyll` (más `robots.txt` y `sitemap.xml` si existen en la raíz, porque los crea `golive/golive.sh`). `tests/`, `golive/`, `scripts/`, `.github/` y este README quedan fuera y dejan de ser URLs públicas.
