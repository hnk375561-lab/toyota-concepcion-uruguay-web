document.addEventListener("DOMContentLoaded", function(){
  ...
  MODELOS_TOYOTA.forEach(function(m){
    ...
    el.innerHTML =
      ...
      '<div style="display:flex;gap:12px;border-top:1px solid var(--border);padding-top:11px;"><a target="_blank" rel="noopener" href="' + wa("ventas", "Hola! Quiero consultar por la " + m.nombre + ".") + '" aria-label="Consultar por ' + m.nombre + '" style="border:0;padding:0;flex:1;">Consultar <span>&rsaquo;</span></a><button class="compare-btn" type="button" data-compare-model="' + m.nombre + '" style="border:0;background:none;color:var(--muted);font:inherit;cursor:pointer;">Comparar</button></div>';
  ...
  SERVICIOS.forEach(function(s){
    ...
    el.innerHTML =
      '<h3>' + s.titulo + '</h3>' +
      '<p>' + s.desc + '</p>' +
      '<a class="btn primary" target="_blank" rel="noopener" href="' + wa(s.numero, s.msg) + '" aria-label="Escribir por ' + s.titulo + '">Escribir</a>';
  ...
  function readRecent(){try{return JSON.parse(sessionStorage.getItem(recentKey)||"[]").filter(function(n){return Object.prototype.hasOwnProperty.call(MODEL_DETAILS,n);});}catch(e){return[];}}
